...
$(ROOTFS_DIR).base:
	...
	mkdir -p $@.tmp
	cat $(shell echo multistrap.list.in; for i in $(DISTS); do echo dists/$$i/multistrap.list.in; done | xargs) | perl -pe "s{__DISTS__}{$(DISTS)}g" | perl -pe "s{__SUITE__}{$(DIST)}g" | perl -pe "s{__ARCH__}{$(ARCH)}g" > multistrap.list
